...
const ProductCard: React.FC<ProductCardProps> = ({ product, view = 'grid', className = '' }) => {
  ...
  if (view === 'list') {
    return (
      <div
        className={`flex gap-4 rounded-xl overflow-hidden transition-all duration-200 p-3 cursor-pointer ${className}`}
        style={{
  ...
  return (
    <div
      className={`group relative flex flex-col rounded-xl overflow-hidden transition-all duration-300 cursor-pointer ${className}`}
      style={{
        backgroundColor: colors.white,
        border: `1px solid ${colors.border}`,
    ...
    >
      ...
      <div className="flex flex-col gap-2 p-3 flex-1">
        ...
        <button
          onClick={handleAddToCart}
          className="w-full py-2 rounded-md text-xs font-bold transition-all duration-200 mt-1 cursor-po"
          style={{
            backgroundColor: addedToCart ? colors.primaryHover : colors.primary,
            color: colors.white,
          }}
